...
const LOGIN_PW_BAN_MS = 12 * 60 * 60 * 1000;
...
function AuthModal({ open, onClose, tab, setTab, reason, onSuccess, onGoKvkk, onGoAbout }) {
  ...
  const [loginPwFailCount, setLoginPwFailCount] = useState(0);
  ...
  const [genCodeValue, setGenCodeValue] = useState("");
  const [codeInput, setCodeInput] = useState("");
  const [codeError, setCodeError] = useState("");
  const CODE_TIME_LIMIT = 300; // 5 minutes
  const [timeLeft, setTimeLeft] = useState(CODE_TIME_LIMIT);

  // shared code-verification brute-force ladder (login + register)
  ...
  // timed-out code entry counts as a failed attempt toward the ban ladder
  useEffect(() => {
    if (inVerifyStep && timeLeft === 0) applyCodeStrike();
  }, [inVerifyStep, timeLeft]);
  ...
  function applyCodeStrike() {
    const next = codeFailCount + 1;
    setCodeFailCount(next);
    if (next >= 5) {
      setRequiresRecovery(true);
    } else {
      setCodeBanUntil(Date.now() + BAN_LADDER_MS[next - 1]);
    }
  }

  function handleLoginCaptcha() {
    if (loginCaptcha) return;
    setLoginCaptchaLoading(true);
    setTimeout(() => { setLoginCaptchaLoading(false); setLoginCaptcha(true); }, 700);
  }

  function handleRegCaptcha() {
    if (regCaptcha) return;
    setRegCaptchaLoading(true);
    setTimeout(() => { setRegCaptchaLoading(false); setRegCaptcha(true); }, 700);
  }
  ...
  function handleLoginSubmit() {
    ...
    if (!loginCaptcha) { setLoginError("Lütfen robot olmadığınızı doğrulayın."); return; }
    if (!loginId.trim() || !loginPw) { setLoginError("Lütfen tüm alanları doldurun."); return; }

    const idLower = loginId.trim().toLowerCase();
    const account = FAKE_ACCOUNTS.find((a) => a.username === idLower || a.email === idLower);
    if (account && account.password === loginPw) {
      setLoginPwFailCount(0);
      if (requiresRecovery || Date.now() < codeBanUntil) { setLoginStep("verify"); return; }
      startCodeStep();
      setLoginStep("verify");
      return;
    }

    const nextFails = loginPwFailCount + 1;
    setLoginPwFailCount(nextFails);
    if (nextFails >= 5) {
      setLoginPwBanUntil(Date.now() + LOGIN_PW_BAN_MS);
      setLoginError("Çok fazla hatalı giriş denemesi. Hesabınız 12 saat süreyle geçici olarak kısıtlandı.");
    } else {
      setLoginError(`Kullanıcı adı/e-posta veya şifre hatalı. Kalan deneme hakkınız: ${5 - nextFails}.`);
    }
  }
  ...
  function handleRegisterSubmit() {
    ...
    if (!regCaptcha) { setRegError("Lütfen robot olmadığınızı doğrulayın."); return; }
    ...
    if (REGISTERED_EMAILS.includes(regEmail.trim().toLowerCase())) {
      setRegError("Bu e-posta adresi zaten sisteme kayıtlı. Lütfen hesabınıza giriş yapınız veya şifrenizi unuttuysanız sıfırlayınız.");
      return;
    }
    startCodeStep();
  ...
  function handleVerifySubmit() {
    if (codeInput.trim() === genCodeValue) {
      setCodeFailCount(0);
      onSuccess();
      return;
    }
      ...
      <div className="overlay-in fixed inset-0 z-[70] grid place-items-center p-3 sm:p-4" style={{ backgroundColor: "#0f172a" }}>
        <div className="modal-in relative max-h-[85vh] w-full max-w-sm overflow-y-auto rounded-2xl bg-white p-4 shadow-2xl sm:max-h-[90vh] sm:p-6">
          ...
          {requiresRecovery ? (
            ...
            <>
              <h2 className="mb-1 text-center text-lg font-bold text-slate-900">{tab === "login" ? "Girişi Doğrulayın" : "E-postanızı Doğrulayın"}</h2>
              ...
              <p className="mb-4 rounded-md bg-slate-50 p-2 text-center text-[11px] text-slate-400">
                Beta / test modu — gerçek e-posta gönderimi henüz aktif değil. Kodunuz: <strong className="text-slate-600">{genCodeValue}</strong>
              </p>
              ...
              <button
                ...
                disabled={codeInput.length !== 6 || timeLeft === 0}
                className="w-full rounded-md bg-gradient-to-b from-orange-500 to-orange-600 py-2.5 text-sm font-semibold text-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-orange-500/30 active:scale-95 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:translate-y-0 disabled:hover:shadow-sm"
              >
                {tab === "login" ? "Doğrula ve Giriş Yap" : "Doğrula ve Hesabı Oluştur"}
              </button>
  ...
  return (
    <div className="overlay-in fixed inset-0 z-[70] grid place-items-center p-3 sm:p-4" style={{ backgroundColor: "#0f172a" }}>
      <div className="modal-in relative max-h-[88vh] w-full max-w-sm overflow-y-auto rounded-2xl bg-white p-3.5 shadow-2xl sm:max-h-[90vh] sm:p-6">
        ...
        <div className="mb-2.5 flex w-full items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
          <button
            onClick={tab === "register" ? handleRegCaptcha : handleLoginCaptcha}
            aria-label="Robot değilim"
            className={`flex h-6 w-6 shrink-0 items-center justify-center rounded border-2 transition-all duration-200 ${(tab === "register" ? regCaptcha : loginCaptcha) ? "border-emerald-500 bg-emerald-500" : "border-slate-300 bg-white hover:border-orange-400"}`}
          >
            {(tab === "register" ? regCaptchaLoading : loginCaptchaLoading) ? (
              <span className="h-3 w-3 animate-spin rounded-full border-2 border-slate-300 border-t-orange-500" />
            ) : (tab === "register" ? regCaptcha : loginCaptcha) ? (
              <CheckCircle2 size={14} className="text-white" />
            ) : null}
          </button>
          <span className="text-left text-xs font-medium text-slate-600">Ben bir robot değilim</span>
        ...
        <button onClick={tab === "register" ? handleRegisterSubmit : handleLoginSubmit} disabled={tab === "login" && (!loginCaptcha || Date.now() < loginPwBanUntil)} className="mt-1 w-full rounded-md bg-gradient-to-b from-orange-500 to-orange-600 py-2.5 text-sm font-semibold text-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-orange-500/30 active:scale-95 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:translate-y-0 disabled:hover:shadow-sm">
